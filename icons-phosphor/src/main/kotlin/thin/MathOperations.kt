package composeicons.phosphor.thin

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.PathNode
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.phosphor.PhosphorIcons
import composeicons.phosphor.phosphorThinIcon

val PhosphorIcons.Thin.MathOperations: ImageVector
    get() {
        if (_mathOperations != null) return _mathOperations!!
        _mathOperations = phosphorThinIcon(
            name = "MathOperations",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 256.0f, height = 256.0f),
        ) {
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(108.0f, 72.0f),
                    PathNode.CurveTo(108.0f, 74.20914f, 106.20914f, 76.0f, 104.0f, 76.0f),
                    PathNode.LineTo(40.0f, 76.0f),
                    PathNode.CurveTo(37.79086f, 76.0f, 36.0f, 74.20914f, 36.0f, 72.0f),
                    PathNode.CurveTo(36.0f, 69.79086f, 37.79086f, 68.0f, 40.0f, 68.0f),
                    PathNode.LineTo(104.0f, 68.0f),
                    PathNode.CurveTo(106.20914f, 68.0f, 108.0f, 69.79086f, 108.0f, 72.0f),
                    PathNode.Close,
                    PathNode.MoveTo(104.0f, 180.0f),
                    PathNode.LineTo(76.0f, 180.0f),
                    PathNode.LineTo(76.0f, 152.0f),
                    PathNode.CurveTo(76.0f, 149.79086f, 74.20914f, 148.0f, 72.0f, 148.0f),
                    PathNode.CurveTo(69.79086f, 148.0f, 68.0f, 149.79086f, 68.0f, 152.0f),
                    PathNode.LineTo(68.0f, 180.0f),
                    PathNode.LineTo(40.0f, 180.0f),
                    PathNode.CurveTo(37.79086f, 180.0f, 36.0f, 181.79086f, 36.0f, 184.0f),
                    PathNode.CurveTo(36.0f, 186.20914f, 37.79086f, 188.0f, 40.0f, 188.0f),
                    PathNode.LineTo(68.0f, 188.0f),
                    PathNode.LineTo(68.0f, 216.0f),
                    PathNode.CurveTo(68.0f, 218.20914f, 69.79086f, 220.0f, 72.0f, 220.0f),
                    PathNode.CurveTo(74.20914f, 220.0f, 76.0f, 218.20914f, 76.0f, 216.0f),
                    PathNode.LineTo(76.0f, 188.0f),
                    PathNode.LineTo(104.0f, 188.0f),
                    PathNode.CurveTo(106.20914f, 188.0f, 108.0f, 186.20914f, 108.0f, 184.0f),
                    PathNode.CurveTo(108.0f, 181.79086f, 106.20914f, 180.0f, 104.0f, 180.0f),
                    PathNode.Close,
                    PathNode.MoveTo(152.0f, 172.0f),
                    PathNode.LineTo(216.0f, 172.0f),
                    PathNode.CurveTo(218.20914f, 172.0f, 220.0f, 170.20914f, 220.0f, 168.0f),
                    PathNode.CurveTo(220.0f, 165.79086f, 218.20914f, 164.0f, 216.0f, 164.0f),
                    PathNode.LineTo(152.0f, 164.0f),
                    PathNode.CurveTo(149.79086f, 164.0f, 148.0f, 165.79086f, 148.0f, 168.0f),
                    PathNode.CurveTo(148.0f, 170.20914f, 149.79086f, 172.0f, 152.0f, 172.0f),
                    PathNode.Close,
                    PathNode.MoveTo(216.0f, 196.0f),
                    PathNode.LineTo(152.0f, 196.0f),
                    PathNode.CurveTo(149.79086f, 196.0f, 148.0f, 197.79086f, 148.0f, 200.0f),
                    PathNode.CurveTo(148.0f, 202.20914f, 149.79086f, 204.0f, 152.0f, 204.0f),
                    PathNode.LineTo(216.0f, 204.0f),
                    PathNode.CurveTo(218.20914f, 204.0f, 220.0f, 202.20914f, 220.0f, 200.0f),
                    PathNode.CurveTo(220.0f, 197.79086f, 218.20914f, 196.0f, 216.0f, 196.0f),
                    PathNode.Close,
                    PathNode.MoveTo(157.17f, 98.83f),
                    PathNode.CurveTo(157.92027f, 99.581116f, 158.93837f, 100.00314f, 160.0f, 100.00314f),
                    PathNode.CurveTo(161.06163f, 100.00314f, 162.07973f, 99.581116f, 162.83f, 98.83f),
                    PathNode.LineTo(184.0f, 77.66f),
                    PathNode.LineTo(205.17f, 98.83f),
                    PathNode.CurveTo(206.73297f, 100.39297f, 209.26703f, 100.39297f, 210.83f, 98.83f),
                    PathNode.CurveTo(212.39296f, 97.26704f, 212.39296f, 94.73296f, 210.83f, 93.17f),
                    PathNode.LineTo(189.66f, 72.0f),
                    PathNode.LineTo(210.83f, 50.83f),
                    PathNode.CurveTo(212.39296f, 49.267033f, 212.39296f, 46.732967f, 210.83f, 45.17f),
                    PathNode.CurveTo(209.26703f, 43.607033f, 206.73297f, 43.607033f, 205.17f, 45.17f),
                    PathNode.LineTo(184.0f, 66.34f),
                    PathNode.LineTo(162.83f, 45.17f),
                    PathNode.CurveTo(161.26703f, 43.607033f, 158.73297f, 43.607033f, 157.17f, 45.17f),
                    PathNode.CurveTo(155.60704f, 46.732967f, 155.60704f, 49.267033f, 157.17f, 50.83f),
                    PathNode.LineTo(178.34f, 72.0f),
                    PathNode.LineTo(157.17f, 93.17f),
                    PathNode.CurveTo(156.41888f, 93.92027f, 155.99686f, 94.93836f, 155.99686f, 96.0f),
                    PathNode.CurveTo(155.99686f, 97.06164f, 156.41888f, 98.07973f, 157.17f, 98.83f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = SolidColor(Color.Black),
                fillAlpha = 1f,
                stroke = null,
                strokeAlpha = 1f,
                strokeLineWidth = 8.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
        }
        return _mathOperations!!
    }

private var _mathOperations: ImageVector? = null
