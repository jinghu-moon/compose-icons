package composeicons.phosphor.duotone

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.PathNode
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.phosphor.PhosphorIcons
import composeicons.phosphor.phosphorDuotoneIcon

val PhosphorIcons.Duotone.CloudSnow: ImageVector
    get() {
        if (_cloudSnow != null) return _cloudSnow!!
        _cloudSnow = phosphorDuotoneIcon(
            name = "CloudSnow",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 256.0f, height = 256.0f),
        ) {
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(224.0f, 92.0f),
                    PathNode.CurveTo(224.0f, 129.55536f, 193.55536f, 160.0f, 156.0f, 160.0f),
                    PathNode.LineTo(76.0f, 160.0f),
                    PathNode.CurveTo(58.55578f, 159.98326f, 42.76869f, 149.66266f, 35.755142f, 133.69044f),
                    PathNode.CurveTo(28.741598f, 117.71825f, 31.826166f, 99.11092f, 43.6184f, 86.25618f),
                    PathNode.CurveTo(55.410637f, 73.40144f, 73.68348f, 68.72701f, 90.2f, 74.34f),
                    PathNode.LineTo(90.2f, 74.45f),
                    PathNode.CurveTo(99.03797f, 41.472816f, 130.94037f, 20.146421f, 164.79137f, 24.58652f),
                    PathNode.CurveTo(198.64236f, 29.026617f, 223.96564f, 57.85907f, 224.0f, 92.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = SolidColor(Color.Black),
                fillAlpha = 1f,
                stroke = null,
                strokeAlpha = 1f,
                strokeLineWidth = 16.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(88.0f, 196.0f),
                    PathNode.CurveTo(88.0f, 202.62741f, 82.62742f, 208.0f, 76.0f, 208.0f),
                    PathNode.CurveTo(69.37258f, 208.0f, 64.0f, 202.62741f, 64.0f, 196.0f),
                    PathNode.CurveTo(64.0f, 189.37259f, 69.37258f, 184.0f, 76.0f, 184.0f),
                    PathNode.CurveTo(82.62742f, 184.0f, 88.0f, 189.37259f, 88.0f, 196.0f),
                    PathNode.Close,
                    PathNode.MoveTo(116.0f, 200.0f),
                    PathNode.CurveTo(109.37258f, 200.0f, 104.0f, 205.37259f, 104.0f, 212.0f),
                    PathNode.CurveTo(104.0f, 218.62741f, 109.37258f, 224.0f, 116.0f, 224.0f),
                    PathNode.CurveTo(122.62742f, 224.0f, 128.0f, 218.62741f, 128.0f, 212.0f),
                    PathNode.CurveTo(128.0f, 205.37259f, 122.62742f, 200.0f, 116.0f, 200.0f),
                    PathNode.Close,
                    PathNode.MoveTo(164.0f, 184.0f),
                    PathNode.CurveTo(157.37259f, 184.0f, 152.0f, 189.37259f, 152.0f, 196.0f),
                    PathNode.CurveTo(152.0f, 202.62741f, 157.37259f, 208.0f, 164.0f, 208.0f),
                    PathNode.CurveTo(170.62741f, 208.0f, 176.0f, 202.62741f, 176.0f, 196.0f),
                    PathNode.CurveTo(176.0f, 189.37259f, 170.62741f, 184.0f, 164.0f, 184.0f),
                    PathNode.Close,
                    PathNode.MoveTo(68.0f, 224.0f),
                    PathNode.CurveTo(61.37258f, 224.0f, 56.0f, 229.37259f, 56.0f, 236.0f),
                    PathNode.CurveTo(56.0f, 242.62741f, 61.37258f, 248.0f, 68.0f, 248.0f),
                    PathNode.CurveTo(74.62742f, 248.0f, 80.0f, 242.62741f, 80.0f, 236.0f),
                    PathNode.CurveTo(80.0f, 229.37259f, 74.62742f, 224.0f, 68.0f, 224.0f),
                    PathNode.Close,
                    PathNode.MoveTo(156.0f, 224.0f),
                    PathNode.CurveTo(149.37259f, 224.0f, 144.0f, 229.37259f, 144.0f, 236.0f),
                    PathNode.CurveTo(144.0f, 242.62741f, 149.37259f, 248.0f, 156.0f, 248.0f),
                    PathNode.CurveTo(162.62741f, 248.0f, 168.0f, 242.62741f, 168.0f, 236.0f),
                    PathNode.CurveTo(168.0f, 229.37259f, 162.62741f, 224.0f, 156.0f, 224.0f),
                    PathNode.Close,
                    PathNode.MoveTo(232.0f, 92.0f),
                    PathNode.CurveTo(231.9559f, 133.95537f, 197.95538f, 167.9559f, 156.0f, 168.0f),
                    PathNode.LineTo(76.0f, 168.0f),
                    PathNode.CurveTo(47.281193f, 168.0f, 24.0f, 144.71881f, 24.0f, 116.0f),
                    PathNode.CurveTo(24.0f, 87.2812f, 47.281193f, 64.0f, 76.0f, 64.0f),
                    PathNode.CurveTo(78.989395f, 64.002556f, 81.97327f, 64.25678f, 84.92f, 64.76f),
                    PathNode.CurveTo(98.08372f, 30.567497f, 133.7486f, 10.614112f, 169.77486f, 17.286367f),
                    PathNode.CurveTo(205.80113f, 23.958624f, 231.95464f, 55.361107f, 232.0f, 92.0f),
                    PathNode.Close,
                    PathNode.MoveTo(216.0f, 92.0f),
                    PathNode.CurveTo(215.86697f, 59.61541f, 190.08229f, 33.167774f, 157.7115f, 32.212837f),
                    PathNode.CurveTo(125.34072f, 31.257898f, 98.04183f, 56.139565f, 96.0f, 88.46f),
                    PathNode.CurveTo(95.74595f, 92.87828f, 91.958275f, 96.25405f, 87.54f, 96.0f),
                    PathNode.CurveTo(83.12172f, 95.74595f, 79.74595f, 91.958275f, 80.0f, 87.54f),
                    PathNode.QuadTo(80.21f, 83.88f, 80.77f, 80.31f),
                    PathNode.CurveTo(79.18806f, 80.10696f, 77.59492f, 80.00342f, 76.0f, 80.0f),
                    PathNode.CurveTo(56.11775f, 80.0f, 40.0f, 96.11775f, 40.0f, 116.0f),
                    PathNode.CurveTo(40.0f, 135.88223f, 56.11775f, 152.0f, 76.0f, 152.0f),
                    PathNode.LineTo(156.0f, 152.0f),
                    PathNode.CurveTo(189.1211f, 151.96143f, 215.96143f, 125.12109f, 216.0f, 92.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = SolidColor(Color.Black),
                fillAlpha = 1f,
                stroke = null,
                strokeAlpha = 1f,
                strokeLineWidth = 16.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
        }
        return _cloudSnow!!
    }

private var _cloudSnow: ImageVector? = null
