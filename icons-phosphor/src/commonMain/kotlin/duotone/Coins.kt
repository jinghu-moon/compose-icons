package composeicons.phosphor.duotone

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.PathNode
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.phosphor.PhosphorIcons
import composeicons.phosphor.phosphorDuotoneIcon

val PhosphorIcons.Duotone.Coins: ImageVector
    get() {
        if (_coins != null) return _coins!!
        _coins = phosphorDuotoneIcon(
            name = "Coins",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 256.0f, height = 256.0f),
        ) {
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(240.0f, 132.0f),
                    PathNode.CurveTo(240.0f, 151.88f, 204.18f, 168.0f, 160.0f, 168.0f),
                    PathNode.CurveTo(140.4f, 168.0f, 122.44f, 164.83f, 108.53f, 159.56f),
                    PathNode.LineTo(108.53f, 159.56f),
                    PathNode.CurveTo(146.76f, 156.85f, 176.0f, 142.0f, 176.0f, 124.0f),
                    PathNode.LineTo(176.0f, 96.72f),
                    PathNode.LineTo(176.0f, 96.72f),
                    PathNode.CurveTo(212.52f, 100.06f, 240.0f, 114.58f, 240.0f, 132.0f),
                    PathNode.Close,
                    PathNode.MoveTo(176.0f, 84.0f),
                    PathNode.CurveTo(176.0f, 64.12f, 140.18f, 48.0f, 96.0f, 48.0f),
                    PathNode.CurveTo(51.82f, 48.0f, 16.0f, 64.12f, 16.0f, 84.0f),
                    PathNode.CurveTo(16.0f, 103.88f, 51.82f, 120.0f, 96.0f, 120.0f),
                    PathNode.CurveTo(140.18f, 120.0f, 176.0f, 103.88f, 176.0f, 84.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = SolidColor(Color.Black),
                fillAlpha = 0.2f,
                stroke = null,
                strokeAlpha = 0.2f,
                strokeLineWidth = 16.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(184.0f, 89.57f),
                    PathNode.LineTo(184.0f, 84.0f),
                    PathNode.CurveTo(184.0f, 58.92f, 146.17f, 40.0f, 96.0f, 40.0f),
                    PathNode.CurveTo(45.83f, 40.0f, 8.0f, 58.92f, 8.0f, 84.0f),
                    PathNode.LineTo(8.0f, 124.0f),
                    PathNode.CurveTo(8.0f, 144.89f, 34.25f, 161.49f, 72.0f, 166.46f),
                    PathNode.LineTo(72.0f, 172.0f),
                    PathNode.CurveTo(72.0f, 197.08f, 109.83f, 216.0f, 160.0f, 216.0f),
                    PathNode.CurveTo(210.17f, 216.0f, 248.0f, 197.08f, 248.0f, 172.0f),
                    PathNode.LineTo(248.0f, 132.0f),
                    PathNode.CurveTo(248.0f, 111.3f, 222.58f, 94.68f, 184.0f, 89.57f),
                    PathNode.Close,
                    PathNode.MoveTo(232.0f, 132.0f),
                    PathNode.CurveTo(232.0f, 145.22f, 201.21f, 160.0f, 160.0f, 160.0f),
                    PathNode.CurveTo(156.27f, 160.0f, 152.57f, 159.87f, 148.92f, 159.63f),
                    PathNode.CurveTo(170.49f, 151.77f, 184.0f, 139.0f, 184.0f, 124.0f),
                    PathNode.LineTo(184.0f, 105.74f),
                    PathNode.CurveTo(213.87f, 110.19f, 232.0f, 122.27f, 232.0f, 132.0f),
                    PathNode.Close,
                    PathNode.MoveTo(72.0f, 150.25f),
                    PathNode.LineTo(72.0f, 126.46f),
                    PathNode.CurveTo(79.95779f, 127.496895f, 87.97495f, 128.01134f, 96.0f, 128.0f),
                    PathNode.CurveTo(104.02505f, 128.01134f, 112.04221f, 127.496895f, 120.0f, 126.46f),
                    PathNode.LineTo(120.0f, 150.25f),
                    PathNode.CurveTo(112.05405f, 151.42386f, 104.03219f, 152.00879f, 96.0f, 152.0f),
                    PathNode.CurveTo(87.96781f, 152.00879f, 79.94595f, 151.42386f, 72.0f, 150.25f),
                    PathNode.Close,
                    PathNode.MoveTo(168.0f, 109.93f),
                    PathNode.LineTo(168.0f, 124.0f),
                    PathNode.CurveTo(168.0f, 132.39f, 155.59f, 141.4f, 136.0f, 146.87f),
                    PathNode.LineTo(136.0f, 123.5f),
                    PathNode.CurveTo(148.91f, 120.37f, 159.84f, 115.71f, 168.0f, 109.93f),
                    PathNode.Close,
                    PathNode.MoveTo(96.0f, 56.0f),
                    PathNode.CurveTo(137.21f, 56.0f, 168.0f, 70.78f, 168.0f, 84.0f),
                    PathNode.CurveTo(168.0f, 97.22f, 137.21f, 112.0f, 96.0f, 112.0f),
                    PathNode.CurveTo(54.79f, 112.0f, 24.0f, 97.22f, 24.0f, 84.0f),
                    PathNode.CurveTo(24.0f, 70.78f, 54.79f, 56.0f, 96.0f, 56.0f),
                    PathNode.Close,
                    PathNode.MoveTo(24.0f, 124.0f),
                    PathNode.LineTo(24.0f, 109.93f),
                    PathNode.CurveTo(32.16f, 115.71f, 43.09f, 120.37f, 56.0f, 123.5f),
                    PathNode.LineTo(56.0f, 146.87f),
                    PathNode.CurveTo(36.41f, 141.4f, 24.0f, 132.39f, 24.0f, 124.0f),
                    PathNode.Close,
                    PathNode.MoveTo(88.0f, 172.0f),
                    PathNode.LineTo(88.0f, 167.83f),
                    PathNode.CurveTo(90.63f, 167.93f, 93.29f, 168.0f, 96.0f, 168.0f),
                    PathNode.CurveTo(99.88f, 168.0f, 103.67f, 167.87f, 107.39f, 167.65f),
                    PathNode.CurveTo(111.52258f, 169.12941f, 115.73205f, 170.38458f, 120.0f, 171.41f),
                    PathNode.LineTo(120.0f, 194.87f),
                    PathNode.CurveTo(100.41f, 189.4f, 88.0f, 180.39f, 88.0f, 172.0f),
                    PathNode.Close,
                    PathNode.MoveTo(136.0f, 198.25f),
                    PathNode.LineTo(136.0f, 174.4f),
                    PathNode.CurveTo(143.95522f, 175.4694f, 151.97322f, 176.00394f, 160.0f, 176.0f),
                    PathNode.CurveTo(168.02504f, 176.01134f, 176.04222f, 175.4969f, 184.0f, 174.46f),
                    PathNode.LineTo(184.0f, 198.25f),
                    PathNode.CurveTo(168.08507f, 200.58328f, 151.91493f, 200.58328f, 136.0f, 198.25f),
                    PathNode.Close,
                    PathNode.MoveTo(200.0f, 194.87f),
                    PathNode.LineTo(200.0f, 171.5f),
                    PathNode.CurveTo(212.91f, 168.37f, 223.84f, 163.71f, 232.0f, 157.93f),
                    PathNode.LineTo(232.0f, 172.0f),
                    PathNode.CurveTo(232.0f, 180.39f, 219.59f, 189.4f, 200.0f, 194.87f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = SolidColor(Color.Black),
                fillAlpha = 1.0f,
                stroke = null,
                strokeAlpha = 1.0f,
                strokeLineWidth = 16.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
        }
        return _coins!!
    }

private var _coins: ImageVector? = null
