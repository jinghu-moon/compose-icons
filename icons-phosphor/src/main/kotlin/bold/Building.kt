package composeicons.phosphor.bold

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.PathNode
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.phosphor.PhosphorIcons
import composeicons.phosphor.phosphorBoldIcon

val PhosphorIcons.Bold.Building: ImageVector
    get() {
        if (_building != null) return _building!!
        _building = phosphorBoldIcon(
            name = "Building",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 256.0f, height = 256.0f),
        ) {
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(232.0f, 220.0f),
                    PathNode.LineTo(212.0f, 220.0f),
                    PathNode.LineTo(212.0f, 36.0f),
                    PathNode.LineTo(216.0f, 36.0f),
                    PathNode.CurveTo(222.62741f, 36.0f, 228.0f, 30.627417f, 228.0f, 24.0f),
                    PathNode.CurveTo(228.0f, 17.372583f, 222.62741f, 12.0f, 216.0f, 12.0f),
                    PathNode.LineTo(40.0f, 12.0f),
                    PathNode.CurveTo(33.37258f, 12.0f, 28.0f, 17.372583f, 28.0f, 24.0f),
                    PathNode.CurveTo(28.0f, 30.627417f, 33.37258f, 36.0f, 40.0f, 36.0f),
                    PathNode.LineTo(44.0f, 36.0f),
                    PathNode.LineTo(44.0f, 220.0f),
                    PathNode.LineTo(24.0f, 220.0f),
                    PathNode.CurveTo(17.372583f, 220.0f, 12.0f, 225.37259f, 12.0f, 232.0f),
                    PathNode.CurveTo(12.0f, 238.62741f, 17.372583f, 244.0f, 24.0f, 244.0f),
                    PathNode.LineTo(232.0f, 244.0f),
                    PathNode.CurveTo(238.62741f, 244.0f, 244.0f, 238.62741f, 244.0f, 232.0f),
                    PathNode.CurveTo(244.0f, 225.37259f, 238.62741f, 220.0f, 232.0f, 220.0f),
                    PathNode.Close,
                    PathNode.MoveTo(68.0f, 36.0f),
                    PathNode.LineTo(188.0f, 36.0f),
                    PathNode.LineTo(188.0f, 220.0f),
                    PathNode.LineTo(164.0f, 220.0f),
                    PathNode.LineTo(164.0f, 184.0f),
                    PathNode.CurveTo(164.0f, 177.37259f, 158.62741f, 172.0f, 152.0f, 172.0f),
                    PathNode.LineTo(104.0f, 172.0f),
                    PathNode.CurveTo(97.37258f, 172.0f, 92.0f, 177.37259f, 92.0f, 184.0f),
                    PathNode.LineTo(92.0f, 220.0f),
                    PathNode.LineTo(68.0f, 220.0f),
                    PathNode.Close,
                    PathNode.MoveTo(140.0f, 220.0f),
                    PathNode.LineTo(116.0f, 220.0f),
                    PathNode.LineTo(116.0f, 196.0f),
                    PathNode.LineTo(140.0f, 196.0f),
                    PathNode.Close,
                    PathNode.MoveTo(84.0f, 64.0f),
                    PathNode.CurveTo(84.0f, 57.37258f, 89.37258f, 52.0f, 96.0f, 52.0f),
                    PathNode.LineTo(108.0f, 52.0f),
                    PathNode.CurveTo(114.62742f, 52.0f, 120.0f, 57.37258f, 120.0f, 64.0f),
                    PathNode.CurveTo(120.0f, 70.62742f, 114.62742f, 76.0f, 108.0f, 76.0f),
                    PathNode.LineTo(96.0f, 76.0f),
                    PathNode.CurveTo(89.37258f, 76.0f, 84.0f, 70.62742f, 84.0f, 64.0f),
                    PathNode.Close,
                    PathNode.MoveTo(136.0f, 64.0f),
                    PathNode.CurveTo(136.0f, 57.37258f, 141.37259f, 52.0f, 148.0f, 52.0f),
                    PathNode.LineTo(160.0f, 52.0f),
                    PathNode.CurveTo(166.62741f, 52.0f, 172.0f, 57.37258f, 172.0f, 64.0f),
                    PathNode.CurveTo(172.0f, 70.62742f, 166.62741f, 76.0f, 160.0f, 76.0f),
                    PathNode.LineTo(148.0f, 76.0f),
                    PathNode.CurveTo(141.37259f, 76.0f, 136.0f, 70.62742f, 136.0f, 64.0f),
                    PathNode.Close,
                    PathNode.MoveTo(84.0f, 104.0f),
                    PathNode.CurveTo(84.0f, 97.37258f, 89.37258f, 92.0f, 96.0f, 92.0f),
                    PathNode.LineTo(108.0f, 92.0f),
                    PathNode.CurveTo(114.62742f, 92.0f, 120.0f, 97.37258f, 120.0f, 104.0f),
                    PathNode.CurveTo(120.0f, 110.62742f, 114.62742f, 116.0f, 108.0f, 116.0f),
                    PathNode.LineTo(96.0f, 116.0f),
                    PathNode.CurveTo(89.37258f, 116.0f, 84.0f, 110.62742f, 84.0f, 104.0f),
                    PathNode.Close,
                    PathNode.MoveTo(136.0f, 104.0f),
                    PathNode.CurveTo(136.0f, 97.37258f, 141.37259f, 92.0f, 148.0f, 92.0f),
                    PathNode.LineTo(160.0f, 92.0f),
                    PathNode.CurveTo(166.62741f, 92.0f, 172.0f, 97.37258f, 172.0f, 104.0f),
                    PathNode.CurveTo(172.0f, 110.62742f, 166.62741f, 116.0f, 160.0f, 116.0f),
                    PathNode.LineTo(148.0f, 116.0f),
                    PathNode.CurveTo(141.37259f, 116.0f, 136.0f, 110.62742f, 136.0f, 104.0f),
                    PathNode.Close,
                    PathNode.MoveTo(84.0f, 144.0f),
                    PathNode.CurveTo(84.0f, 137.37259f, 89.37258f, 132.0f, 96.0f, 132.0f),
                    PathNode.LineTo(108.0f, 132.0f),
                    PathNode.CurveTo(114.62742f, 132.0f, 120.0f, 137.37259f, 120.0f, 144.0f),
                    PathNode.CurveTo(120.0f, 150.62741f, 114.62742f, 156.0f, 108.0f, 156.0f),
                    PathNode.LineTo(96.0f, 156.0f),
                    PathNode.CurveTo(89.37258f, 156.0f, 84.0f, 150.62741f, 84.0f, 144.0f),
                    PathNode.Close,
                    PathNode.MoveTo(136.0f, 144.0f),
                    PathNode.CurveTo(136.0f, 137.37259f, 141.37259f, 132.0f, 148.0f, 132.0f),
                    PathNode.LineTo(160.0f, 132.0f),
                    PathNode.CurveTo(166.62741f, 132.0f, 172.0f, 137.37259f, 172.0f, 144.0f),
                    PathNode.CurveTo(172.0f, 150.62741f, 166.62741f, 156.0f, 160.0f, 156.0f),
                    PathNode.LineTo(148.0f, 156.0f),
                    PathNode.CurveTo(141.37259f, 156.0f, 136.0f, 150.62741f, 136.0f, 144.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = SolidColor(Color.Black),
                fillAlpha = 1f,
                stroke = null,
                strokeAlpha = 1f,
                strokeLineWidth = 24.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
        }
        return _building!!
    }

private var _building: ImageVector? = null
