package composeicons.lucide.outline

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.core.parseSvgPathData
import composeicons.lucide.LucideIcons
import composeicons.lucide.lucideOutlineIcon

val LucideIcons.Outline.WeightTilde: ImageVector
    get() {
        if (_weightTilde != null) return _weightTilde!!
        _weightTilde = lucideOutlineIcon(
            name = "WeightTilde",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 24.0f, height = 24.0f),
        ) {
    addPath(
        pathData = parseSvgPathData("M 6.500 8.000 C 5.611 8.009 4.834 8.604 4.594 9.460 L 2.100 18.500 C 1.947 19.093 2.074 19.723 2.444 20.210 C 2.815 20.698 3.388 20.989 4.000 21.000 L 20.000 21.000 C 20.626 21.000 21.215 20.707 21.593 20.208 C 21.971 19.709 22.094 19.062 21.925 18.460 L 19.400 9.500 C 19.174 8.623 18.386 8.007 17.480 8.000 Z"),
        pathFillType = PathFillType.NonZero,
        fill = null,
        fillAlpha = 1.0f,
        stroke = SolidColor(Color(0xFF000000)),
        strokeAlpha = 1.0f,
        strokeLineWidth = 2.0f,
        strokeLineCap = StrokeCap.Round,
        strokeLineJoin = StrokeJoin.Round,
    )
    addPath(
        pathData = parseSvgPathData("M 7.999 15.000 C 8.471 14.370 9.212 14.000 9.999 14.000 C 10.786 14.000 11.527 14.370 11.999 15.000 C 12.471 15.630 13.212 16.000 13.999 16.000 C 14.786 16.000 15.527 15.630 15.999 15.000"),
        pathFillType = PathFillType.NonZero,
        fill = null,
        fillAlpha = 1.0f,
        stroke = SolidColor(Color(0xFF000000)),
        strokeAlpha = 1.0f,
        strokeLineWidth = 2.0f,
        strokeLineCap = StrokeCap.Round,
        strokeLineJoin = StrokeJoin.Round,
    )
    addPath(
        pathData = parseSvgPathData("M 15.000 5.000 C 15.000 6.657 13.657 8.000 12.000 8.000 C 10.343 8.000 9.000 6.657 9.000 5.000 C 9.000 3.343 10.343 2.000 12.000 2.000 C 13.657 2.000 15.000 3.343 15.000 5.000 Z"),
        pathFillType = PathFillType.NonZero,
        fill = null,
        fillAlpha = 1.0f,
        stroke = SolidColor(Color(0xFF000000)),
        strokeAlpha = 1.0f,
        strokeLineWidth = 2.0f,
        strokeLineCap = StrokeCap.Round,
        strokeLineJoin = StrokeJoin.Round,
    )
        }
        return _weightTilde!!
    }

private var _weightTilde: ImageVector? = null
