package composeicons.phosphor.light

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.PathNode
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.phosphor.PhosphorIcons
import composeicons.phosphor.phosphorLightIcon

val PhosphorIcons.Light.SelectionForeground: ImageVector
    get() {
        if (_selectionForeground != null) return _selectionForeground!!
        _selectionForeground = phosphorLightIcon(
            name = "SelectionForeground",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 256.0f, height = 256.0f),
        ) {
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(62.0f, 216.0f),
                    PathNode.CurveTo(62.0f, 219.3137f, 59.31371f, 222.0f, 56.0f, 222.0f),
                    PathNode.LineTo(48.0f, 222.0f),
                    PathNode.CurveTo(40.268013f, 222.0f, 34.0f, 215.73198f, 34.0f, 208.0f),
                    PathNode.LineTo(34.0f, 200.0f),
                    PathNode.CurveTo(34.0f, 196.6863f, 36.68629f, 194.0f, 40.0f, 194.0f),
                    PathNode.CurveTo(43.31371f, 194.0f, 46.0f, 196.6863f, 46.0f, 200.0f),
                    PathNode.LineTo(46.0f, 208.0f),
                    PathNode.CurveTo(46.0f, 209.10457f, 46.89543f, 210.0f, 48.0f, 210.0f),
                    PathNode.LineTo(56.0f, 210.0f),
                    PathNode.CurveTo(59.31371f, 210.0f, 62.0f, 212.6863f, 62.0f, 216.0f),
                    PathNode.Close,
                    PathNode.MoveTo(112.0f, 210.0f),
                    PathNode.LineTo(96.0f, 210.0f),
                    PathNode.CurveTo(92.686295f, 210.0f, 90.0f, 212.6863f, 90.0f, 216.0f),
                    PathNode.CurveTo(90.0f, 219.3137f, 92.686295f, 222.0f, 96.0f, 222.0f),
                    PathNode.LineTo(112.0f, 222.0f),
                    PathNode.CurveTo(115.313705f, 222.0f, 118.0f, 219.3137f, 118.0f, 216.0f),
                    PathNode.CurveTo(118.0f, 212.6863f, 115.313705f, 210.0f, 112.0f, 210.0f),
                    PathNode.Close,
                    PathNode.MoveTo(40.0f, 166.0f),
                    PathNode.CurveTo(43.31371f, 166.0f, 46.0f, 163.3137f, 46.0f, 160.0f),
                    PathNode.LineTo(46.0f, 144.0f),
                    PathNode.CurveTo(46.0f, 140.6863f, 43.31371f, 138.0f, 40.0f, 138.0f),
                    PathNode.CurveTo(36.68629f, 138.0f, 34.0f, 140.6863f, 34.0f, 144.0f),
                    PathNode.LineTo(34.0f, 160.0f),
                    PathNode.CurveTo(34.0f, 163.3137f, 36.68629f, 166.0f, 40.0f, 166.0f),
                    PathNode.Close,
                    PathNode.MoveTo(168.0f, 194.0f),
                    PathNode.CurveTo(164.6863f, 194.0f, 162.0f, 196.6863f, 162.0f, 200.0f),
                    PathNode.LineTo(162.0f, 208.0f),
                    PathNode.CurveTo(162.0f, 209.10457f, 161.10457f, 210.0f, 160.0f, 210.0f),
                    PathNode.LineTo(152.0f, 210.0f),
                    PathNode.CurveTo(148.6863f, 210.0f, 146.0f, 212.6863f, 146.0f, 216.0f),
                    PathNode.CurveTo(146.0f, 219.3137f, 148.6863f, 222.0f, 152.0f, 222.0f),
                    PathNode.LineTo(160.0f, 222.0f),
                    PathNode.CurveTo(167.73198f, 222.0f, 174.0f, 215.73198f, 174.0f, 208.0f),
                    PathNode.LineTo(174.0f, 200.0f),
                    PathNode.CurveTo(174.0f, 196.6863f, 171.3137f, 194.0f, 168.0f, 194.0f),
                    PathNode.Close,
                    PathNode.MoveTo(168.0f, 110.0f),
                    PathNode.CurveTo(171.3137f, 110.0f, 174.0f, 107.313705f, 174.0f, 104.0f),
                    PathNode.LineTo(174.0f, 96.0f),
                    PathNode.CurveTo(174.0f, 88.26801f, 167.73198f, 82.0f, 160.0f, 82.0f),
                    PathNode.LineTo(152.0f, 82.0f),
                    PathNode.CurveTo(148.6863f, 82.0f, 146.0f, 84.686295f, 146.0f, 88.0f),
                    PathNode.CurveTo(146.0f, 91.313705f, 148.6863f, 94.0f, 152.0f, 94.0f),
                    PathNode.LineTo(160.0f, 94.0f),
                    PathNode.CurveTo(161.10457f, 94.0f, 162.0f, 94.89543f, 162.0f, 96.0f),
                    PathNode.LineTo(162.0f, 104.0f),
                    PathNode.CurveTo(162.0f, 107.313705f, 164.6863f, 110.0f, 168.0f, 110.0f),
                    PathNode.Close,
                    PathNode.MoveTo(56.0f, 82.0f),
                    PathNode.LineTo(48.0f, 82.0f),
                    PathNode.CurveTo(40.268013f, 82.0f, 34.0f, 88.26801f, 34.0f, 96.0f),
                    PathNode.LineTo(34.0f, 104.0f),
                    PathNode.CurveTo(34.0f, 107.313705f, 36.68629f, 110.0f, 40.0f, 110.0f),
                    PathNode.CurveTo(43.31371f, 110.0f, 46.0f, 107.313705f, 46.0f, 104.0f),
                    PathNode.LineTo(46.0f, 96.0f),
                    PathNode.CurveTo(46.0f, 94.89543f, 46.89543f, 94.0f, 48.0f, 94.0f),
                    PathNode.LineTo(56.0f, 94.0f),
                    PathNode.CurveTo(59.31371f, 94.0f, 62.0f, 91.313705f, 62.0f, 88.0f),
                    PathNode.CurveTo(62.0f, 84.686295f, 59.31371f, 82.0f, 56.0f, 82.0f),
                    PathNode.Close,
                    PathNode.MoveTo(208.0f, 34.0f),
                    PathNode.LineTo(96.0f, 34.0f),
                    PathNode.CurveTo(88.26801f, 34.0f, 82.0f, 40.268013f, 82.0f, 48.0f),
                    PathNode.LineTo(82.0f, 88.0f),
                    PathNode.LineTo(82.0f, 88.0f),
                    PathNode.CurveTo(82.0f, 91.313705f, 84.686295f, 94.0f, 88.0f, 94.0f),
                    PathNode.LineTo(112.0f, 94.0f),
                    PathNode.CurveTo(115.313705f, 94.0f, 118.0f, 91.313705f, 118.0f, 88.0f),
                    PathNode.CurveTo(118.0f, 84.686295f, 115.313705f, 82.0f, 112.0f, 82.0f),
                    PathNode.LineTo(94.0f, 82.0f),
                    PathNode.LineTo(94.0f, 48.0f),
                    PathNode.CurveTo(94.0f, 46.89543f, 94.89543f, 46.0f, 96.0f, 46.0f),
                    PathNode.LineTo(208.0f, 46.0f),
                    PathNode.CurveTo(209.10457f, 46.0f, 210.0f, 46.89543f, 210.0f, 48.0f),
                    PathNode.LineTo(210.0f, 160.0f),
                    PathNode.CurveTo(210.0f, 161.10457f, 209.10457f, 162.0f, 208.0f, 162.0f),
                    PathNode.LineTo(174.0f, 162.0f),
                    PathNode.LineTo(174.0f, 144.0f),
                    PathNode.CurveTo(174.0f, 140.6863f, 171.3137f, 138.0f, 168.0f, 138.0f),
                    PathNode.CurveTo(164.6863f, 138.0f, 162.0f, 140.6863f, 162.0f, 144.0f),
                    PathNode.LineTo(162.0f, 168.0f),
                    PathNode.CurveTo(162.0f, 171.3137f, 164.6863f, 174.0f, 168.0f, 174.0f),
                    PathNode.LineTo(208.0f, 174.0f),
                    PathNode.CurveTo(215.73198f, 174.0f, 222.0f, 167.73198f, 222.0f, 160.0f),
                    PathNode.LineTo(222.0f, 48.0f),
                    PathNode.CurveTo(222.0f, 40.268013f, 215.73198f, 34.0f, 208.0f, 34.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = SolidColor(Color.Black),
                fillAlpha = 1f,
                stroke = null,
                strokeAlpha = 1f,
                strokeLineWidth = 12.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
        }
        return _selectionForeground!!
    }

private var _selectionForeground: ImageVector? = null
