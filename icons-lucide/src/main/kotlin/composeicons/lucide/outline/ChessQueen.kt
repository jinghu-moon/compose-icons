package composeicons.lucide.outline

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.PathNode
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.lucide.LucideIcons
import composeicons.lucide.lucideOutlineIcon

val LucideIcons.Outline.ChessQueen: ImageVector
    get() {
        if (_chessQueen != null) return _chessQueen!!
        _chessQueen = lucideOutlineIcon(
            name = "ChessQueen",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 24.0f, height = 24.0f),
        ) {
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(4.0f, 20.0f),
                    PathNode.CurveTo(4.0f, 18.89543f, 4.895431f, 18.0f, 6.0f, 18.0f),
                    PathNode.LineTo(18.0f, 18.0f),
                    PathNode.CurveTo(19.10457f, 18.0f, 20.0f, 18.89543f, 20.0f, 20.0f),
                    PathNode.LineTo(20.0f, 21.0f),
                    PathNode.CurveTo(20.0f, 21.552284f, 19.552284f, 22.0f, 19.0f, 22.0f),
                    PathNode.LineTo(5.0f, 22.0f),
                    PathNode.CurveTo(4.447716f, 22.0f, 4.0f, 21.552284f, 4.0f, 21.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(12.474f, 5.943f),
                    PathNode.LineTo(14.041f, 11.283f),
                    PathNode.CurveTo(14.147511f, 11.643004f, 14.446688f, 11.91302f, 14.815692f, 11.982183f),
                    PathNode.CurveTo(15.184695f, 12.051344f, 15.561349f, 11.907998f, 15.791f, 11.611f),
                    PathNode.LineTo(18.407f, 8.209f)
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(20.0f, 9.0f),
                    PathNode.LineTo(17.0f, 18.0f)
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(5.594f, 8.209f),
                    PathNode.LineTo(8.209f, 11.612f),
                    PathNode.CurveTo(8.4388f, 11.908976f, 8.815599f, 12.052183f, 9.184637f, 11.982804f),
                    PathNode.CurveTo(9.553675f, 11.913424f, 9.85274f, 11.643154f, 9.959f, 11.283f),
                    PathNode.LineTo(11.526f, 5.943f)
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(7.0f, 18.0f),
                    PathNode.LineTo(4.0f, 9.0f)
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(14.0f, 4.0f),
                    PathNode.CurveTo(14.0f, 5.10457f, 13.104569f, 6.0f, 12.0f, 6.0f),
                    PathNode.CurveTo(10.895431f, 6.0f, 10.0f, 5.10457f, 10.0f, 4.0f),
                    PathNode.CurveTo(10.0f, 2.895431f, 10.895431f, 2.0f, 12.0f, 2.0f),
                    PathNode.CurveTo(13.104569f, 2.0f, 14.0f, 2.895431f, 14.0f, 4.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(22.0f, 7.0f),
                    PathNode.CurveTo(22.0f, 8.10457f, 21.10457f, 9.0f, 20.0f, 9.0f),
                    PathNode.CurveTo(18.89543f, 9.0f, 18.0f, 8.10457f, 18.0f, 7.0f),
                    PathNode.CurveTo(18.0f, 5.895431f, 18.89543f, 5.0f, 20.0f, 5.0f),
                    PathNode.CurveTo(21.10457f, 5.0f, 22.0f, 5.895431f, 22.0f, 7.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(6.0f, 7.0f),
                    PathNode.CurveTo(6.0f, 8.10457f, 5.10457f, 9.0f, 4.0f, 9.0f),
                    PathNode.CurveTo(2.895431f, 9.0f, 2.0f, 8.10457f, 2.0f, 7.0f),
                    PathNode.CurveTo(2.0f, 5.895431f, 2.895431f, 5.0f, 4.0f, 5.0f),
                    PathNode.CurveTo(5.10457f, 5.0f, 6.0f, 5.895431f, 6.0f, 7.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
        }
        return _chessQueen!!
    }

private var _chessQueen: ImageVector? = null
