package composeicons.phosphor.light

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.PathNode
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.phosphor.PhosphorIcons
import composeicons.phosphor.phosphorLightIcon

val PhosphorIcons.Light.Exclude: ImageVector
    get() {
        if (_exclude != null) return _exclude!!
        _exclude = phosphorLightIcon(
            name = "Exclude",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 256.0f, height = 256.0f),
        ) {
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(172.91f, 83.09f),
                    PathNode.CurveTo(166.34064f, 43.893257f, 131.35025f, 15.903645f, 91.66769f, 18.102404f),
                    PathNode.CurveTo(51.985115f, 20.301163f, 20.301163f, 51.985115f, 18.102404f, 91.66769f),
                    PathNode.CurveTo(15.903645f, 131.35025f, 43.893257f, 166.34064f, 83.09f, 172.91f),
                    PathNode.CurveTo(89.659355f, 212.10674f, 124.64974f, 240.09636f, 164.33232f, 237.8976f),
                    PathNode.CurveTo(204.0149f, 235.69884f, 235.69884f, 204.0149f, 237.8976f, 164.33232f),
                    PathNode.CurveTo(240.09636f, 124.64974f, 212.10674f, 89.659355f, 172.91f, 83.09f),
                    PathNode.Close,
                    PathNode.MoveTo(226.0f, 160.0f),
                    PathNode.CurveTo(225.99644f, 162.97733f, 225.78929f, 165.95093f, 225.38f, 168.9f),
                    PathNode.LineTo(171.62f, 115.13f),
                    PathNode.CurveTo(173.20293f, 108.87658f, 174.0024f, 102.45065f, 174.0f, 96.0f),
                    PathNode.CurveTo(174.0f, 95.83f, 174.0f, 95.67f, 174.0f, 95.51f),
                    PathNode.CurveTo(204.3251f, 102.13111f, 225.95837f, 128.96051f, 226.0f, 160.0f),
                    PathNode.Close,
                    PathNode.MoveTo(45.31f, 53.79f),
                    PathNode.LineTo(100.81f, 109.29f),
                    PathNode.CurveTo(95.91226f, 115.00973f, 91.87017f, 121.409706f, 88.81f, 128.29f),
                    PathNode.LineTo(34.0f, 73.48f),
                    PathNode.CurveTo(36.604164f, 66.31255f, 40.430786f, 59.650646f, 45.31f, 53.79f),
                    PathNode.Close,
                    PathNode.MoveTo(73.48f, 34.0f),
                    PathNode.LineTo(128.28f, 88.81f),
                    PathNode.CurveTo(121.399704f, 91.87017f, 114.999725f, 95.91226f, 109.28f, 100.81f),
                    PathNode.LineTo(53.78f, 45.31f),
                    PathNode.CurveTo(59.643665f, 40.429787f, 66.30903f, 36.603123f, 73.48f, 34.0f),
                    PathNode.Close,
                    PathNode.MoveTo(94.0f, 160.0f),
                    PathNode.CurveTo(94.04408f, 123.56748f, 123.56748f, 94.04408f, 160.0f, 94.0f),
                    PathNode.CurveTo(160.65f, 94.0f, 161.3f, 94.0f, 161.95f, 94.0f),
                    PathNode.CurveTo(161.95f, 94.65f, 162.0f, 95.3f, 162.0f, 96.0f),
                    PathNode.CurveTo(161.95592f, 132.43253f, 132.43253f, 161.95592f, 96.0f, 162.0f),
                    PathNode.CurveTo(95.35f, 162.0f, 94.7f, 162.0f, 94.0f, 161.95f),
                    PathNode.CurveTo(94.0f, 161.3f, 94.0f, 160.65f, 94.0f, 160.0f),
                    PathNode.Close,
                    PathNode.MoveTo(146.71f, 155.19f),
                    PathNode.LineTo(202.21f, 210.69f),
                    PathNode.CurveTo(196.34935f, 215.56921f, 189.68744f, 219.39584f, 182.52f, 222.0f),
                    PathNode.LineTo(127.72f, 167.19f),
                    PathNode.CurveTo(134.59679f, 164.12898f, 140.99335f, 160.08691f, 146.71f, 155.19f),
                    PathNode.Close,
                    PathNode.MoveTo(155.19f, 146.71f),
                    PathNode.CurveTo(160.08774f, 140.99026f, 164.12982f, 134.5903f, 167.19f, 127.71f),
                    PathNode.LineTo(222.0f, 182.52f),
                    PathNode.CurveTo(219.38367f, 189.69025f, 215.54341f, 196.35234f, 210.65f, 202.21f),
                    PathNode.Close,
                    PathNode.MoveTo(160.49f, 82.01f),
                    PathNode.LineTo(160.0f, 82.01f),
                    PathNode.CurveTo(153.54935f, 82.007614f, 147.12341f, 82.807076f, 140.87f, 84.39f),
                    PathNode.LineTo(87.1f, 30.62f),
                    PathNode.CurveTo(90.049065f, 30.21071f, 93.02267f, 30.00356f, 96.0f, 30.0f),
                    PathNode.CurveTo(127.03948f, 30.041628f, 153.8689f, 51.674892f, 160.49f, 82.0f),
                    PathNode.Close,
                    PathNode.MoveTo(30.0f, 96.0f),
                    PathNode.CurveTo(30.00356f, 93.02267f, 30.21071f, 90.049065f, 30.62f, 87.1f),
                    PathNode.LineTo(84.38f, 140.87f),
                    PathNode.CurveTo(82.79707f, 147.12341f, 81.997604f, 153.54935f, 82.0f, 160.0f),
                    PathNode.CurveTo(82.0f, 160.17f, 82.0f, 160.33f, 82.0f, 160.49f),
                    PathNode.CurveTo(51.674892f, 153.8689f, 30.041628f, 127.03948f, 30.0f, 96.0f),
                    PathNode.Close,
                    PathNode.MoveTo(95.51f, 174.0f),
                    PathNode.LineTo(96.0f, 174.0f),
                    PathNode.CurveTo(102.45065f, 174.0024f, 108.87658f, 173.20293f, 115.13f, 171.62f),
                    PathNode.LineTo(168.9f, 225.38f),
                    PathNode.CurveTo(165.95093f, 225.78929f, 162.97733f, 225.99644f, 160.0f, 226.0f),
                    PathNode.CurveTo(128.96051f, 225.95837f, 102.13111f, 204.3251f, 95.51f, 174.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = SolidColor(Color.Black),
                fillAlpha = 1.0f,
                stroke = null,
                strokeAlpha = 1.0f,
                strokeLineWidth = 12.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
        }
        return _exclude!!
    }

private var _exclude: ImageVector? = null
