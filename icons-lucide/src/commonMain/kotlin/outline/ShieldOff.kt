package composeicons.lucide.outline

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.PathNode
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.lucide.LucideIcons
import composeicons.lucide.lucideOutlineIcon

val LucideIcons.Outline.ShieldOff: ImageVector
    get() {
        if (_shieldOff != null) return _shieldOff!!
        _shieldOff = lucideOutlineIcon(
            name = "ShieldOff",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 24.0f, height = 24.0f),
        ) {
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(2.0f, 2.0f),
                    PathNode.LineTo(22.0f, 22.0f)
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1.0f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1.0f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(5.0f, 5.0f),
                    PathNode.CurveTo(4.447716f, 5.0f, 4.0f, 5.447716f, 4.0f, 6.0f),
                    PathNode.LineTo(4.0f, 13.0f),
                    PathNode.CurveTo(4.0f, 18.0f, 7.5f, 20.5f, 11.67f, 21.94f),
                    PathNode.CurveTo(11.885537f, 22.020287f, 12.122163f, 22.023817f, 12.34f, 21.95f),
                    PathNode.CurveTo(14.69f, 21.13f, 16.82f, 19.98f, 18.24f, 18.24f)
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1.0f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1.0f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(9.309f, 3.652f),
                    PathNode.CurveTo(9.99547f, 3.258109f, 10.642159f, 2.798629f, 11.24f, 2.28f),
                    PathNode.CurveTo(11.677673f, 1.906067f, 12.322327f, 1.906067f, 12.76f, 2.28f),
                    PathNode.CurveTo(14.51f, 3.81f, 17.0f, 5.0f, 19.0f, 5.0f),
                    PathNode.CurveTo(19.552284f, 5.0f, 20.0f, 5.447716f, 20.0f, 6.0f),
                    PathNode.LineTo(20.0f, 13.0f),
                    PathNode.CurveTo(20.00066f, 13.422619f, 19.973938f, 13.844836f, 19.92f, 14.264f)
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1.0f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1.0f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
        }
        return _shieldOff!!
    }

private var _shieldOff: ImageVector? = null
