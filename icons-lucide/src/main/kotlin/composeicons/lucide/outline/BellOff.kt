package composeicons.lucide.outline

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.PathNode
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.lucide.LucideIcons
import composeicons.lucide.lucideOutlineIcon

val LucideIcons.Outline.BellOff: ImageVector
    get() {
        if (_bellOff != null) return _bellOff!!
        _bellOff = lucideOutlineIcon(
            name = "BellOff",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 24.0f, height = 24.0f),
        ) {
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(10.268f, 21.0f),
                    PathNode.CurveTo(10.625278f, 21.618753f, 11.285505f, 21.999912f, 12.0f, 21.999912f),
                    PathNode.CurveTo(12.714495f, 21.999912f, 13.374722f, 21.618753f, 13.732f, 21.0f)
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(17.0f, 17.0f),
                    PathNode.LineTo(4.0f, 17.0f),
                    PathNode.CurveTo(3.604038f, 17.000141f, 3.245284f, 16.766636f, 3.085111f, 16.404514f),
                    PathNode.CurveTo(2.924938f, 16.042395f, 2.993517f, 15.619871f, 3.26f, 15.327f),
                    PathNode.CurveTo(4.59f, 13.956f, 6.0f, 12.499f, 6.0f, 8.0f),
                    PathNode.CurveTo(5.999855f, 7.409805f, 6.086792f, 6.822818f, 6.258f, 6.258f)
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(2.0f, 2.0f),
                    PathNode.LineTo(22.0f, 22.0f)
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(8.668f, 3.01f),
                    PathNode.CurveTo(10.508919f, 1.780696f, 12.877155f, 1.664882f, 14.829237f, 2.708697f),
                    PathNode.CurveTo(16.781319f, 3.752513f, 18.000086f, 5.786367f, 18.0f, 8.0f),
                    PathNode.CurveTo(18.0f, 10.687f, 18.77f, 12.653f, 19.707f, 14.05f)
                ),
                pathFillType = PathFillType.NonZero,
                fill = null,
                fillAlpha = 1f,
                stroke = SolidColor(Color.Black),
                strokeAlpha = 1f,
                strokeLineWidth = 2.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
        }
        return _bellOff!!
    }

private var _bellOff: ImageVector? = null
