...
| **本项目** | **usvg 拍平 → 绝对坐标 path → 直接 emit** | **零冗余，零中间格式** |

任何"原始 SVG 长得乱七八糟"的复杂度，由 [usvg](https://github.com/linebender/resvg/tree/main/crates/usvg)（Rust 实现的 SVG 标准化预处理器）一次性消化掉。Kotlin 端只面对**已经被简化到极致**的 path 数据。
...
**Why**：这些都是 usvg 的职责。Kotlin 端重新实现一份等于在维护两份 SVG 解析器。

**正确做法**：generator-core 的 `NormalizedPathParser` 只识别 5 种 path 命令（M / L / Q / C / Z），全部绝对坐标。任何复杂 SVG 输入由 usvg 在前置步骤拍平。
...
│                                                                      │
│  ┌─────────────────────┐                                             │
│  │ tools/usvg(.exe)    │  ← 第二层解耦：SVG 标准化预处理器             │
│  │ (Rust binary)       │     由 Gradle 任务自动下载                   │
│  └──────────┬──────────┘                                             │
│             │ stdin: 原始 SVG / stdout: 拍平 SVG                       │
│             ▼                                                        │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │ generator/                                                     │  │
│  │ ├─ core/   generator-core（图标源无关的纯逻辑）                │  │
│  │ │  ├─ IconSource              ← 第一层解耦                     │  │
│  │ │  ├─ UsvgPipeline            ← ProcessBuilder 调用 usvg       │  │
│  │ │  ├─ NormalizedPathParser    ← 仅消费 usvg 输出（5 种命令）   │  │
│  │ │  ├─ PathAttributes          ← 第三层解耦                     │  │
│  │ │  ├─ KotlinFileGenerator     ← emit ImageVector .kt           │  │
...
| Phosphor (v2) | `assets/{regular,thin,light,bold,fill,duotone}/*.svg` 六个子目录 |
...
fun generate(source: IconSource, config: GeneratorConfig): GeneratorReport {
    ...
    icons.forEach { icon ->
        // 此时已经抹平了文件夹结构差异
        val rawSvg     = icon.file.readText()
        val flattened  = usvgPipeline.flatten(rawSvg)        // 第二层
        val parsedPath = normalizedPathParser.parse(flattened)
        val finalAttrs = mergeAttributes(                    // 第三层
            raw     = parsedPath.attrs,
            default = source.defaultPathAttributes(icon.style),
        )
        kotlinFileGenerator.emit(icon, parsedPath, finalAttrs)
    }
}
```
...
- 添加新图标源 = 新增一个 `IconSource` 实现 + 一个 `generator/<source>/Main.kt` 入口，**不修改 generator-core**
...
直接降维打击：引入 [usvg](https://github.com/linebender/resvg/tree/main/crates/usvg)，作为构建期外部 CLI 工具。在 Kotlin 开始干活前，先让 usvg 把所有 SVG 拍平到统一形态。
...
> - All filters are supported (但 v1 仍按"包含 filter 的图标判失败"处理，因为 Compose 不渲染 filter)
> - Recursive elements will be detected and removed
> - `objectBoundingBox` will be replaced with `userSpaceOnUse`

**经过 usvg 拍平后，所有 SVG 都变成同一种模样**：仅含绝对坐标 path 的扁平结构。

#### Kotlin 端 NormalizedPathParser 的全部职责

只识别五种 path 命令（M / L / Q / C / Z），按 SVG 规范提取数字。**没有**：
- `<g>` / `<use>` / `<defs>` / `<style>` / `<symbol>` / `<switch>` / `<marker>` 的处理
- transform 解析与坐标变换
- 基础形状到 path 的转换
- CSS 解析与样式继承
- relative 命令 / 隐式命令 / ArcTo 命令

代码量从当前 `SvgIconParser` + `SvgPathDataParser` + `SvgBasicShapeConverter` + `SvgValidator` 的 ~400 行 + 测试，降到 **~80 行** + 测试。

#### 集成方式

按 [Q7 决策](#) 选择 **U7-A：外部 CLI + 进程调用**：

1. **二进制位置**：`tools/usvg(.exe)`（git ignore）
2. **下载机制**：Gradle 任务 `:tools:resolveUsvg` 检查二进制是否存在，不存在则从 [resvg releases](https://github.com/linebender/resvg/releases) 按 OS 自动下载
3. **调用方式**：`ProcessBuilder` 启动 `usvg - -c --indent none --coordinates-precision 6`，stdin 写入原始 SVG，stdout 读取拍平结果
4. **批量优化**：generator 启动一个长生命周期的 usvg 进程池（worker pool），通过 stdin/stdout 流式处理 6092 个 SVG，避免每次进程启动开销

#### 契约

- generator-core 的输入永远是 usvg 拍平结果，不是原始 SVG
- 任何"原始 SVG 长得乱七八糟"的问题归 usvg 处理；usvg 处理不了的（如 filter / mask / animation），**该图标判失败而非在 Kotlin 端 hack**
- `SvgValidator` 不再 fail-closed 拒绝 `<g>` / `<use>`（usvg 已经处理掉了），但仍拒绝 usvg 输出中残留的 `<filter>` / `<mask>` / `<clipPath>`（v1 不支持的视觉特性）
...
// ❌ generator-core 内嵌 SVG 结构处理（这是 usvg 的活）
if (svgElement.tagName == "g") {
    applyTransform(svgElement.getAttribute("transform"), children)
}
...
// ❌ 在 Kotlin 中绕过 usvg 自己处理 SVG 复杂特性
fun parseGroupTransform(svg: String) {
...
- **图标源**：Tabler 3.41.1 + Lucide main
- **运行时模块**：`icons-core` / `icons-tabler` / `icons-lucide` 各独立 artifact
- **不发布**：catalog（任何形式）
- **保真度验证**：Paparazzi 100-300 个代表图标采样
- **生成器**：usvg + IconSource 三层解耦完整落地
...
- `KotlinFileGenerator` 改写 emit 逻辑：
...
- 工作量：仅改 emit 模板 + 跑 generator 重新输出，不动 parser

#### 5.2.2 新图标源接入

- **Phosphor v2**（6 weight × 9000 = ~54000 个图标）
...
- `icons-tabler-catalog` / `icons-lucide-catalog` / `icons-phosphor-catalog` 各自发布到 Maven
...
| 阶段 | 单图标耗时 | 全量（6092 + 1703 = 7795 张）|
|------|----------|----------------------------|
| usvg 拍平（独立进程，每图标） | ~30ms 进程启动 + 5ms 处理 | 极差，~4 分钟 |
| usvg 拍平（worker pool 流式） | ~5ms | **~40s（目标）** |
| NormalizedPathParser | < 1ms | < 8s |
| KotlinFileGenerator emit | < 5ms | < 60s |
| **完整 fresh build** | — | **目标 < 90s** |

CI 缓存命中后增量构建 < 5s（仅重新生成上游变更的图标）。
...
- 使用方写 `Icon(TablerIcons.Outline.Home, null)` → R8 静态可达性分析保留：
  ...
  - 其引用的 `_home` 私有字段
  - lambda 内的 `tablerOutlineIcon(...) { addPath(...) }` 调用链
  - `addPath` 引用的 `PathNode` 常量数组
- 未引用的 5038 个 outline + 1053 个 filled icons → R8 全部移除
- AAR 中包含全部 6092 个 .kt 编译产物（class 文件），但 release APK dex 中只剩被引用的部分
...
| `<g>` / transform / `<use>` | 部分支持 | 部分支持 | **完整支持（usvg 处理）** |
...
### 8.2 ResvgLocator 用途重新定位

`refer/resvg.exe` 与新接入的 `tools/usvg.exe` 是**不同**工具：

| 工具 | 用途 | 何时调用 |
|------|------|---------|
| `tools/usvg(.exe)` | SVG 拍平（仅简化结构，输出仍是 SVG） | generator pipeline 第一步 |
| `refer/resvg(.exe)` | SVG 渲染为 PNG | 仅用于 v1 Phase 9 之外的人工对比验证 |

`ResvgLocator` 重命名为 `UsvgLocator`，定位指向 `tools/usvg(.exe)`，并在不存在时通过 Gradle 任务自动下载。

### 8.3 web-preview 数据来源切换

当前 web-preview 消费 `web-preview/public/data/{source}.json`，由 `WebPreviewDataExporter` 生成。改造后：

- generator 输出 `meta.json`（IconMeta + 拍平后 path 数据）
- web-preview 直接消费 meta.json
- `WebPreviewDataExporter` 改名 `MetaJsonExporter`，仅负责输出 meta.json
- 删除 v1 不需要的"复制完整 svg markup 到 JSON" 逻辑（picker UI 应该自己重渲染 path 数据）

### 8.4 SvgValidator 角色淡化

usvg 拍平已经处理了 `<g>` / `<use>` / 嵌套 `<svg>` / 基础形状 / transform / CSS。`SvgValidator` 仅保留对 v1 不支持视觉特性的检测：

- ❌ `<filter>`（Compose 不支持）
- ❌ `<mask>` / `<clipPath>`（usvg 在某些 case 会保留）
- ❌ `<animate>` / `<set>` 等动画元素

usvg 输出中残留这些 → 该图标判失败，写入 generation-report.txt。
