package composeicons.phosphor.thin

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.PathNode
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.phosphor.PhosphorIcons
import composeicons.phosphor.phosphorThinIcon

val PhosphorIcons.Thin.CashRegister: ImageVector
    get() {
        if (_cashRegister != null) return _cashRegister!!
        _cashRegister = phosphorThinIcon(
            name = "CashRegister",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 256.0f, height = 256.0f),
        ) {
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(235.88f, 159.0f),
                    PathNode.LineTo(213.4f, 69.09f),
                    PathNode.CurveTo(212.06392f, 63.744827f, 207.25963f, 59.996243f, 201.75f, 60.0f),
                    PathNode.LineTo(132.0f, 60.0f),
                    PathNode.LineTo(132.0f, 40.0f),
                    PathNode.CurveTo(132.0f, 33.37258f, 126.62742f, 28.0f, 120.0f, 28.0f),
                    PathNode.LineTo(80.0f, 28.0f),
                    PathNode.CurveTo(73.37258f, 28.0f, 68.0f, 33.37258f, 68.0f, 40.0f),
                    PathNode.LineTo(68.0f, 60.0f),
                    PathNode.LineTo(54.25f, 60.0f),
                    PathNode.CurveTo(48.74037f, 59.996243f, 43.936085f, 63.744827f, 42.6f, 69.09f),
                    PathNode.LineTo(20.12f, 159.0f),
                    PathNode.CurveTo(20.038027f, 159.32695f, 19.997707f, 159.66295f, 20.0f, 160.0f),
                    PathNode.LineTo(20.0f, 192.0f),
                    PathNode.CurveTo(20.0f, 198.62741f, 25.372583f, 204.0f, 32.0f, 204.0f),
                    PathNode.LineTo(224.0f, 204.0f),
                    PathNode.CurveTo(230.62741f, 204.0f, 236.0f, 198.62741f, 236.0f, 192.0f),
                    PathNode.LineTo(236.0f, 160.0f),
                    PathNode.CurveTo(236.00229f, 159.66295f, 235.96198f, 159.32695f, 235.88f, 159.0f),
                    PathNode.Close,
                    PathNode.MoveTo(76.0f, 40.0f),
                    PathNode.CurveTo(76.0f, 37.79086f, 77.79086f, 36.0f, 80.0f, 36.0f),
                    PathNode.LineTo(120.0f, 36.0f),
                    PathNode.CurveTo(122.20914f, 36.0f, 124.0f, 37.79086f, 124.0f, 40.0f),
                    PathNode.LineTo(124.0f, 60.0f),
                    PathNode.LineTo(76.0f, 60.0f),
                    PathNode.Close,
                    PathNode.MoveTo(50.37f, 71.0f),
                    PathNode.CurveTo(50.82665f, 69.23139f, 52.423393f, 67.996796f, 54.25f, 68.0f),
                    PathNode.LineTo(201.75f, 68.0f),
                    PathNode.CurveTo(203.5766f, 67.996796f, 205.17336f, 69.23139f, 205.63f, 71.0f),
                    PathNode.LineTo(226.88f, 156.0f),
                    PathNode.LineTo(29.12f, 156.0f),
                    PathNode.Close,
                    PathNode.MoveTo(224.0f, 196.0f),
                    PathNode.LineTo(32.0f, 196.0f),
                    PathNode.CurveTo(29.790861f, 196.0f, 28.0f, 194.20914f, 28.0f, 192.0f),
                    PathNode.LineTo(28.0f, 164.0f),
                    PathNode.LineTo(228.0f, 164.0f),
                    PathNode.LineTo(228.0f, 192.0f),
                    PathNode.CurveTo(228.0f, 194.20914f, 226.20914f, 196.0f, 224.0f, 196.0f),
                    PathNode.Close,
                    PathNode.MoveTo(68.0f, 96.0f),
                    PathNode.CurveTo(68.0f, 93.79086f, 69.79086f, 92.0f, 72.0f, 92.0f),
                    PathNode.LineTo(88.0f, 92.0f),
                    PathNode.CurveTo(90.20914f, 92.0f, 92.0f, 93.79086f, 92.0f, 96.0f),
                    PathNode.CurveTo(92.0f, 98.20914f, 90.20914f, 100.0f, 88.0f, 100.0f),
                    PathNode.LineTo(72.0f, 100.0f),
                    PathNode.CurveTo(69.79086f, 100.0f, 68.0f, 98.20914f, 68.0f, 96.0f),
                    PathNode.Close,
                    PathNode.MoveTo(116.0f, 96.0f),
                    PathNode.CurveTo(116.0f, 93.79086f, 117.79086f, 92.0f, 120.0f, 92.0f),
                    PathNode.LineTo(136.0f, 92.0f),
                    PathNode.CurveTo(138.20914f, 92.0f, 140.0f, 93.79086f, 140.0f, 96.0f),
                    PathNode.CurveTo(140.0f, 98.20914f, 138.20914f, 100.0f, 136.0f, 100.0f),
                    PathNode.LineTo(120.0f, 100.0f),
                    PathNode.CurveTo(117.79086f, 100.0f, 116.0f, 98.20914f, 116.0f, 96.0f),
                    PathNode.Close,
                    PathNode.MoveTo(164.0f, 96.0f),
                    PathNode.CurveTo(164.0f, 93.79086f, 165.79086f, 92.0f, 168.0f, 92.0f),
                    PathNode.LineTo(184.0f, 92.0f),
                    PathNode.CurveTo(186.20914f, 92.0f, 188.0f, 93.79086f, 188.0f, 96.0f),
                    PathNode.CurveTo(188.0f, 98.20914f, 186.20914f, 100.0f, 184.0f, 100.0f),
                    PathNode.LineTo(168.0f, 100.0f),
                    PathNode.CurveTo(165.79086f, 100.0f, 164.0f, 98.20914f, 164.0f, 96.0f),
                    PathNode.Close,
                    PathNode.MoveTo(68.0f, 128.0f),
                    PathNode.CurveTo(68.0f, 125.79086f, 69.79086f, 124.0f, 72.0f, 124.0f),
                    PathNode.LineTo(88.0f, 124.0f),
                    PathNode.CurveTo(90.20914f, 124.0f, 92.0f, 125.79086f, 92.0f, 128.0f),
                    PathNode.CurveTo(92.0f, 130.20914f, 90.20914f, 132.0f, 88.0f, 132.0f),
                    PathNode.LineTo(72.0f, 132.0f),
                    PathNode.CurveTo(69.79086f, 132.0f, 68.0f, 130.20914f, 68.0f, 128.0f),
                    PathNode.Close,
                    PathNode.MoveTo(116.0f, 128.0f),
                    PathNode.CurveTo(116.0f, 125.79086f, 117.79086f, 124.0f, 120.0f, 124.0f),
                    PathNode.LineTo(136.0f, 124.0f),
                    PathNode.CurveTo(138.20914f, 124.0f, 140.0f, 125.79086f, 140.0f, 128.0f),
                    PathNode.CurveTo(140.0f, 130.20914f, 138.20914f, 132.0f, 136.0f, 132.0f),
                    PathNode.LineTo(120.0f, 132.0f),
                    PathNode.CurveTo(117.79086f, 132.0f, 116.0f, 130.20914f, 116.0f, 128.0f),
                    PathNode.Close,
                    PathNode.MoveTo(164.0f, 128.0f),
                    PathNode.CurveTo(164.0f, 125.79086f, 165.79086f, 124.0f, 168.0f, 124.0f),
                    PathNode.LineTo(184.0f, 124.0f),
                    PathNode.CurveTo(186.20914f, 124.0f, 188.0f, 125.79086f, 188.0f, 128.0f),
                    PathNode.CurveTo(188.0f, 130.20914f, 186.20914f, 132.0f, 184.0f, 132.0f),
                    PathNode.LineTo(168.0f, 132.0f),
                    PathNode.CurveTo(165.79086f, 132.0f, 164.0f, 130.20914f, 164.0f, 128.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = SolidColor(Color.Black),
                fillAlpha = 1f,
                stroke = null,
                strokeAlpha = 1f,
                strokeLineWidth = 8.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
        }
        return _cashRegister!!
    }

private var _cashRegister: ImageVector? = null
