package composeicons.phosphor.light

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.PathNode
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.phosphor.PhosphorIcons
import composeicons.phosphor.phosphorLightIcon

val PhosphorIcons.Light.FileDashed: ImageVector
    get() {
        if (_fileDashed != null) return _fileDashed!!
        _fileDashed = phosphorLightIcon(
            name = "FileDashed",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 256.0f, height = 256.0f),
        ) {
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(78.0f, 224.0f),
                    PathNode.CurveTo(78.0f, 227.3137f, 75.313705f, 230.0f, 72.0f, 230.0f),
                    PathNode.LineTo(56.0f, 230.0f),
                    PathNode.CurveTo(48.268013f, 230.0f, 42.0f, 223.73198f, 42.0f, 216.0f),
                    PathNode.LineTo(42.0f, 184.0f),
                    PathNode.CurveTo(42.0f, 180.6863f, 44.68629f, 178.0f, 48.0f, 178.0f),
                    PathNode.CurveTo(51.31371f, 178.0f, 54.0f, 180.6863f, 54.0f, 184.0f),
                    PathNode.LineTo(54.0f, 216.0f),
                    PathNode.CurveTo(54.0f, 217.10457f, 54.89543f, 218.0f, 56.0f, 218.0f),
                    PathNode.LineTo(72.0f, 218.0f),
                    PathNode.CurveTo(75.313705f, 218.0f, 78.0f, 220.6863f, 78.0f, 224.0f),
                    PathNode.Close,
                    PathNode.MoveTo(214.0f, 88.0f),
                    PathNode.LineTo(214.0f, 136.0f),
                    PathNode.CurveTo(214.0f, 139.3137f, 211.3137f, 142.0f, 208.0f, 142.0f),
                    PathNode.CurveTo(204.6863f, 142.0f, 202.0f, 139.3137f, 202.0f, 136.0f),
                    PathNode.LineTo(202.0f, 94.0f),
                    PathNode.LineTo(152.0f, 94.0f),
                    PathNode.CurveTo(148.6863f, 94.0f, 146.0f, 91.313705f, 146.0f, 88.0f),
                    PathNode.LineTo(146.0f, 38.0f),
                    PathNode.LineTo(120.0f, 38.0f),
                    PathNode.CurveTo(116.686295f, 38.0f, 114.0f, 35.31371f, 114.0f, 32.0f),
                    PathNode.CurveTo(114.0f, 28.68629f, 116.686295f, 26.0f, 120.0f, 26.0f),
                    PathNode.LineTo(152.0f, 26.0f),
                    PathNode.CurveTo(153.59076f, 26.0014f, 155.11586f, 26.63446f, 156.24f, 27.76f),
                    PathNode.LineTo(212.24f, 83.76f),
                    PathNode.CurveTo(213.36554f, 84.88415f, 213.9986f, 86.40924f, 214.0f, 88.0f),
                    PathNode.Close,
                    PathNode.MoveTo(158.0f, 82.0f),
                    PathNode.LineTo(193.51f, 82.0f),
                    PathNode.LineTo(158.0f, 46.49f),
                    PathNode.Close,
                    PathNode.MoveTo(80.0f, 26.0f),
                    PathNode.LineTo(56.0f, 26.0f),
                    PathNode.CurveTo(48.268013f, 26.0f, 42.0f, 32.268013f, 42.0f, 40.0f),
                    PathNode.LineTo(42.0f, 64.0f),
                    PathNode.CurveTo(42.0f, 67.313705f, 44.68629f, 70.0f, 48.0f, 70.0f),
                    PathNode.CurveTo(51.31371f, 70.0f, 54.0f, 67.313705f, 54.0f, 64.0f),
                    PathNode.LineTo(54.0f, 40.0f),
                    PathNode.CurveTo(54.0f, 38.89543f, 54.89543f, 38.0f, 56.0f, 38.0f),
                    PathNode.LineTo(80.0f, 38.0f),
                    PathNode.CurveTo(83.313705f, 38.0f, 86.0f, 35.31371f, 86.0f, 32.0f),
                    PathNode.CurveTo(86.0f, 28.68629f, 83.313705f, 26.0f, 80.0f, 26.0f),
                    PathNode.Close,
                    PathNode.MoveTo(208.0f, 170.0f),
                    PathNode.CurveTo(204.6863f, 170.0f, 202.0f, 172.6863f, 202.0f, 176.0f),
                    PathNode.LineTo(202.0f, 216.0f),
                    PathNode.CurveTo(202.0f, 217.10457f, 201.10457f, 218.0f, 200.0f, 218.0f),
                    PathNode.LineTo(192.0f, 218.0f),
                    PathNode.CurveTo(188.6863f, 218.0f, 186.0f, 220.6863f, 186.0f, 224.0f),
                    PathNode.CurveTo(186.0f, 227.3137f, 188.6863f, 230.0f, 192.0f, 230.0f),
                    PathNode.LineTo(200.0f, 230.0f),
                    PathNode.CurveTo(207.73198f, 230.0f, 214.0f, 223.73198f, 214.0f, 216.0f),
                    PathNode.LineTo(214.0f, 176.0f),
                    PathNode.CurveTo(214.0f, 172.6863f, 211.3137f, 170.0f, 208.0f, 170.0f),
                    PathNode.Close,
                    PathNode.MoveTo(48.0f, 150.0f),
                    PathNode.CurveTo(51.31371f, 150.0f, 54.0f, 147.3137f, 54.0f, 144.0f),
                    PathNode.LineTo(54.0f, 104.0f),
                    PathNode.CurveTo(54.0f, 100.686295f, 51.31371f, 98.0f, 48.0f, 98.0f),
                    PathNode.CurveTo(44.68629f, 98.0f, 42.0f, 100.686295f, 42.0f, 104.0f),
                    PathNode.LineTo(42.0f, 144.0f),
                    PathNode.CurveTo(42.0f, 147.3137f, 44.68629f, 150.0f, 48.0f, 150.0f),
                    PathNode.Close,
                    PathNode.MoveTo(152.0f, 218.0f),
                    PathNode.LineTo(112.0f, 218.0f),
                    PathNode.CurveTo(108.686295f, 218.0f, 106.0f, 220.6863f, 106.0f, 224.0f),
                    PathNode.CurveTo(106.0f, 227.3137f, 108.686295f, 230.0f, 112.0f, 230.0f),
                    PathNode.LineTo(152.0f, 230.0f),
                    PathNode.CurveTo(155.3137f, 230.0f, 158.0f, 227.3137f, 158.0f, 224.0f),
                    PathNode.CurveTo(158.0f, 220.6863f, 155.3137f, 218.0f, 152.0f, 218.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = SolidColor(Color.Black),
                fillAlpha = 1f,
                stroke = null,
                strokeAlpha = 1f,
                strokeLineWidth = 12.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
        }
        return _fileDashed!!
    }

private var _fileDashed: ImageVector? = null
