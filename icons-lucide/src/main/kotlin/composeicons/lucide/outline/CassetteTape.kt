package composeicons.lucide.outline

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.lucide.LucideIcons
import composeicons.lucide.lucideOutlineIcon

val LucideIcons.Outline.CassetteTape: ImageVector
    get() {
        if (_cassetteTape != null) return _cassetteTape!!
        _cassetteTape = lucideOutlineIcon(
            name = "CassetteTape",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 24f, height = 24f),
        ) {
            addPathData("M4 4h16c1.105 0 2 .895 2 2v12c0 1.105-.895 2-2 2h-16C2.895 20 2 19.105 2 18v-12C2 4.895 2.895 4 4 4Z")
            addPathData("M10 10c0 1.105-.895 2-2 2C6.895 12 6 11.105 6 10 6 8.895 6.895 8 8 8c1.105 0 2 .895 2 2Z")
            addPathData("M8 12h8")
            addPathData("M18 10c0 1.105-.895 2-2 2-1.105 0-2-.895-2-2 0-1.105 .895-2 2-2 1.105 0 2 .895 2 2Z")
            addPathData("M6 20l.7-2.9C6.843 16.446 7.43 15.984 8.1 16h7.8c.641-.027 1.217 .385 1.4 1L18 20")
        }
        return _cassetteTape!!
    }

private var _cassetteTape: ImageVector? = null
