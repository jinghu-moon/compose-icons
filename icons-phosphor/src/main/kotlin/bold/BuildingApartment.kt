package composeicons.phosphor.bold

import androidx.compose.ui.graphics.*
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.PathNode
import androidx.compose.ui.unit.dp
import composeicons.core.IconSize
import composeicons.core.ViewBox
import composeicons.phosphor.PhosphorIcons
import composeicons.phosphor.phosphorBoldIcon

val PhosphorIcons.Bold.BuildingApartment: ImageVector
    get() {
        if (_buildingApartment != null) return _buildingApartment!!
        _buildingApartment = phosphorBoldIcon(
            name = "BuildingApartment",
            size = IconSize(width = 24f.dp, height = 24f.dp),
            viewBox = ViewBox(minX = 0f, minY = 0f, width = 256.0f, height = 256.0f),
        ) {
            addPath(
                pathData = listOf(
                    PathNode.MoveTo(240.0f, 204.0f),
                    PathNode.LineTo(236.0f, 204.0f),
                    PathNode.LineTo(236.0f, 76.0f),
                    PathNode.CurveTo(236.0f, 69.37258f, 230.62741f, 64.0f, 224.0f, 64.0f),
                    PathNode.LineTo(188.0f, 64.0f),
                    PathNode.LineTo(188.0f, 40.0f),
                    PathNode.CurveTo(188.0f, 33.37258f, 182.62741f, 28.0f, 176.0f, 28.0f),
                    PathNode.LineTo(80.0f, 28.0f),
                    PathNode.CurveTo(73.37258f, 28.0f, 68.0f, 33.37258f, 68.0f, 40.0f),
                    PathNode.LineTo(68.0f, 100.0f),
                    PathNode.LineTo(32.0f, 100.0f),
                    PathNode.CurveTo(25.372583f, 100.0f, 20.0f, 105.37258f, 20.0f, 112.0f),
                    PathNode.LineTo(20.0f, 204.0f),
                    PathNode.LineTo(16.0f, 204.0f),
                    PathNode.CurveTo(9.372583f, 204.0f, 4.0f, 209.37259f, 4.0f, 216.0f),
                    PathNode.CurveTo(4.0f, 222.62741f, 9.372583f, 228.0f, 16.0f, 228.0f),
                    PathNode.LineTo(240.0f, 228.0f),
                    PathNode.CurveTo(246.62741f, 228.0f, 252.0f, 222.62741f, 252.0f, 216.0f),
                    PathNode.CurveTo(252.0f, 209.37259f, 246.62741f, 204.0f, 240.0f, 204.0f),
                    PathNode.Close,
                    PathNode.MoveTo(44.0f, 124.0f),
                    PathNode.LineTo(80.0f, 124.0f),
                    PathNode.CurveTo(86.62742f, 124.0f, 92.0f, 118.62742f, 92.0f, 112.0f),
                    PathNode.LineTo(92.0f, 52.0f),
                    PathNode.LineTo(164.0f, 52.0f),
                    PathNode.LineTo(164.0f, 76.0f),
                    PathNode.CurveTo(164.0f, 82.62742f, 169.37259f, 88.0f, 176.0f, 88.0f),
                    PathNode.LineTo(212.0f, 88.0f),
                    PathNode.LineTo(212.0f, 204.0f),
                    PathNode.LineTo(148.0f, 204.0f),
                    PathNode.LineTo(148.0f, 176.0f),
                    PathNode.CurveTo(148.0f, 173.79086f, 146.20914f, 172.0f, 144.0f, 172.0f),
                    PathNode.LineTo(112.0f, 172.0f),
                    PathNode.CurveTo(109.79086f, 172.0f, 108.0f, 173.79086f, 108.0f, 176.0f),
                    PathNode.LineTo(108.0f, 204.0f),
                    PathNode.LineTo(44.0f, 204.0f),
                    PathNode.Close,
                    PathNode.MoveTo(108.0f, 76.0f),
                    PathNode.CurveTo(108.0f, 69.37258f, 113.37258f, 64.0f, 120.0f, 64.0f),
                    PathNode.LineTo(136.0f, 64.0f),
                    PathNode.CurveTo(142.62741f, 64.0f, 148.0f, 69.37258f, 148.0f, 76.0f),
                    PathNode.CurveTo(148.0f, 82.62742f, 142.62741f, 88.0f, 136.0f, 88.0f),
                    PathNode.LineTo(120.0f, 88.0f),
                    PathNode.CurveTo(113.37258f, 88.0f, 108.0f, 82.62742f, 108.0f, 76.0f),
                    PathNode.Close,
                    PathNode.MoveTo(108.0f, 112.0f),
                    PathNode.CurveTo(108.0f, 105.37258f, 113.37258f, 100.0f, 120.0f, 100.0f),
                    PathNode.LineTo(136.0f, 100.0f),
                    PathNode.CurveTo(142.62741f, 100.0f, 148.0f, 105.37258f, 148.0f, 112.0f),
                    PathNode.CurveTo(148.0f, 118.62742f, 142.62741f, 124.0f, 136.0f, 124.0f),
                    PathNode.LineTo(120.0f, 124.0f),
                    PathNode.CurveTo(113.37258f, 124.0f, 108.0f, 118.62742f, 108.0f, 112.0f),
                    PathNode.Close,
                    PathNode.MoveTo(160.0f, 112.0f),
                    PathNode.CurveTo(160.0f, 105.37258f, 165.37259f, 100.0f, 172.0f, 100.0f),
                    PathNode.LineTo(188.0f, 100.0f),
                    PathNode.CurveTo(194.62741f, 100.0f, 200.0f, 105.37258f, 200.0f, 112.0f),
                    PathNode.CurveTo(200.0f, 118.62742f, 194.62741f, 124.0f, 188.0f, 124.0f),
                    PathNode.LineTo(172.0f, 124.0f),
                    PathNode.CurveTo(165.37259f, 124.0f, 160.0f, 118.62742f, 160.0f, 112.0f),
                    PathNode.Close,
                    PathNode.MoveTo(96.0f, 148.0f),
                    PathNode.CurveTo(96.0f, 154.62741f, 90.62742f, 160.0f, 84.0f, 160.0f),
                    PathNode.LineTo(68.0f, 160.0f),
                    PathNode.CurveTo(61.37258f, 160.0f, 56.0f, 154.62741f, 56.0f, 148.0f),
                    PathNode.CurveTo(56.0f, 141.37259f, 61.37258f, 136.0f, 68.0f, 136.0f),
                    PathNode.LineTo(84.0f, 136.0f),
                    PathNode.CurveTo(90.62742f, 136.0f, 96.0f, 141.37259f, 96.0f, 148.0f),
                    PathNode.Close,
                    PathNode.MoveTo(108.0f, 148.0f),
                    PathNode.CurveTo(108.0f, 141.37259f, 113.37258f, 136.0f, 120.0f, 136.0f),
                    PathNode.LineTo(136.0f, 136.0f),
                    PathNode.CurveTo(142.62741f, 136.0f, 148.0f, 141.37259f, 148.0f, 148.0f),
                    PathNode.CurveTo(148.0f, 154.62741f, 142.62741f, 160.0f, 136.0f, 160.0f),
                    PathNode.LineTo(120.0f, 160.0f),
                    PathNode.CurveTo(113.37258f, 160.0f, 108.0f, 154.62741f, 108.0f, 148.0f),
                    PathNode.Close,
                    PathNode.MoveTo(160.0f, 148.0f),
                    PathNode.CurveTo(160.0f, 141.37259f, 165.37259f, 136.0f, 172.0f, 136.0f),
                    PathNode.LineTo(188.0f, 136.0f),
                    PathNode.CurveTo(194.62741f, 136.0f, 200.0f, 141.37259f, 200.0f, 148.0f),
                    PathNode.CurveTo(200.0f, 154.62741f, 194.62741f, 160.0f, 188.0f, 160.0f),
                    PathNode.LineTo(172.0f, 160.0f),
                    PathNode.CurveTo(165.37259f, 160.0f, 160.0f, 154.62741f, 160.0f, 148.0f),
                    PathNode.Close
                ),
                pathFillType = PathFillType.NonZero,
                fill = SolidColor(Color.Black),
                fillAlpha = 1f,
                stroke = null,
                strokeAlpha = 1f,
                strokeLineWidth = 24.0f,
                strokeLineCap = StrokeCap.Round,
                strokeLineJoin = StrokeJoin.Round,
                strokeLineMiter = 4f,
            )
        }
        return _buildingApartment!!
    }

private var _buildingApartment: ImageVector? = null
